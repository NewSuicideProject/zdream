...
public sealed class Visualizer {
    ...
    private static Vector3 CellCenterWorld(Environment.MapData data, int x, int y) {
        float wx = data.Origin.x + ((x + 0.5f) * data.CellSize);
        float wz = data.Origin.z + ((y + 0.5f) * data.CellSize);
        return new Vector3(wx, 0f, wz);
    }

    private static Vector3 CellTopWorld(Environment.MapData data, int x, int y) {
    ...
    private static float GetWallBaseHeightFromNeighbors(Environment.MapData data, int x, int y) {
