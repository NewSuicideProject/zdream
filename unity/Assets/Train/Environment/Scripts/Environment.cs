    ...
    public class Environment : MonoBehaviour {
        ...
        public void Reset() {
            ...
            foreach (Room room in _map.Rooms) {
                ...
                foreach (Cell cell in room.Cells.Where(cell => _map.InBounds(cell.X, cell.Y))) {
        ...
        private void ConnectRooms() {
            ...
            DisjointSetUnion disjointSetUnion = new(n);
            int picked = 0;

            for (int i = 0; i < edges.Count && picked < n - 1; i++) {
                Edge e = edges[i];
                if (!disjointSetUnion.Union(e.A, e.B)) {
                    continue;
                }
        ...
        private List<Cell> BuildRoadPathCells(Room a, Room b) {
            ...
            bool xThenY = !randomizeLTurnOrder || _rng.NextDouble() < 0.5;
